...
+ (void)showGif:(NSURL *)gifUrl status:(NSString *)status
{
    [[self sharedView] showGif:gifUrl status:status];
}

+ (void)showImages:(NSArray <UIImage *>*)images status:(NSString *)status
{
    [[self sharedView]showGif:images status:status];
}

- (void)showGif:(NSArray <UIImage *> *)images status:(NSString *)status {
    __weak SVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            ...
            // Update imageView
            strongSelf.imageView.bounds = CGRectMake(0, 0, 65, 65);
            strongSelf.imageView.animationImages = images;
            strongSelf.imageView.hidden = NO;
            strongSelf.imageView.animationDuration = images.count / 20.;
