...
class DDVistorGoodsViewModel: DDBaseMoreDataViewModel {
    ...
    override func getMore(pageSize: Int, completion: () -> Void) {
        ...
        Alamofire.request(Router.GoodsList(cat_id: category_id!, brand_id: "", currentPage: "\(currentPage)", pageSize: "\(pageSize)")).responseJSON { [weak self] (response) in
